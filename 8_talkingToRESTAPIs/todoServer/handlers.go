...
var (
	...
	ErrInvaliDate = errors.New("invalid data")
)

func rootHandler(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path != "/" {
		replyError(w, req, http.StatusNotFound, "")
		return
	}
...
func getAllHandler(w http.ResponseWriter, r *http.Request, list *todo.List) {
	...
	replyJSONContent(w, r, http.StatusOK, res)
}
...
func validateID(path string, list *todo.List) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: Invalid ID: %s", ErrInvaliDate, err)
	}
	if id < 1 {
		return 0, fmt.Errorf("%w, Invalid ID: Less than one", ErrInvaliDate)
	}
	if id > len(*list) {
